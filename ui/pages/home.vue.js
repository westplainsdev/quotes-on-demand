        ...
        <blockquote class="blockquote">
                ...
                <footer class="blockquote-footer">{{quote.author}} 
                        <div class="float-right">                
                                <router-link :to="'/manage/' + quote.id" class="btn btn-outline-secondary btn-sm">edit</router-link>
                                <button type="button" title="Delete Quote" class="btn btn-danger btn-sm" @click="removeSingleQuote(quote.id)">
                                        <span>&times;</span>
                                    </button>
                        </div>
                </footer>
  ...
  methods: {
    ...
    getAllQuotes() {
      axios
        ...
        .then(response => {
          ...
          this.selectList.unshift({ author: 'All Authors', value: 0 });
    ...
    setupSelect(quoteList) {
      return quoteList.map(function (quote) {
        return { author: quote.author, value: quote.id };
      });
    }
  },
